...
const numArray = new Uint16Array([...parsed]).sort();

// console.log(numArray);

const sum = numArray.reduce((p, c) => p + c);
console.log('Sum:', sum);

function alignTo(n) {
    let cost = 0;
    numArray.forEach((crab) => { cost += Math.abs(crab - n); });
    return cost;
}

// Part 1:
function puzzle1() {
    const median = numArray.length % 2 === 0 ? (numArray[numArray.length / 2] + numArray[numArray.length / 2 - 1]) / 2 : numArray[Math.floor(numArray.length / 2)];
    console.log('Median:', median);
    let target = median;
    let bestTarget = target;
    let bestCost = alignTo(target);
    // Check lower
    target--;
    let improving = true;
    while (improving) {
        const cost = alignTo(target);
        if (cost < bestCost) {
            bestCost = cost;
            bestTarget = target;
        } else {
...
function alignTwo(n) {
    let cost = 0;
    numArray.forEach((crab) => {
        const steps = Math.abs(crab - n);
        const totalSteps = (steps * (steps + 1)) / 2;
        cost += totalSteps;
    });
    return cost;
}
...
function puzzle2() {
    ...
    let target = Math.floor(mean);
    let bestTarget = target;
    let bestCost = alignTwo(target);
    // Check lower
    target--;
    let improving = true;
    while (improving) {
        const cost = alignTwo(target);
        if (cost <= bestCost) {
            bestCost = cost;
            bestTarget = target;
        } else {
