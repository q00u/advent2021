...
// const allPerms = stringPermutations('abcdefg');
// console.log(allPerms, allPerms.length);
...
// Generate all 5040 possible maps of digits to any segment display
{
    const allPerms = stringPermutations('abcdefg'); // ['gfedcba'];
    allPerms.forEach((perm) => {
        // Generate the digits for this permutation
        const allDigits = [];
        digits.forEach((digit) => {
            let thisDigit = '';
            digit.forEach((segment) => {
                thisDigit += perm[segment];
            });
            allDigits.push(sortChars(thisDigit));
        });
        allMaps.set(allDigits.sort().join('-'), perm);
    });
}
// console.log(allMaps);
...
function puzzle2() {
    ...
    inout.forEach((line) => {
        const [inp, output] = line;
        // Make key from input for this line
        const key = [];
        const inputWords = inp.split(' ');
        inputWords.forEach((digit) => { key.push(sortChars(digit)); });
        // Get matching permutation for this key
        const thisPerm = allMaps.get(key.sort().join('-'));
        ...
        outputWords.forEach((word) => {
            result += interpret(word, thisPerm);
        });
